...
const createAvailibility: RequestHandler = async (req, res, next) => {
  try {
    await caregiver.createAvailibility(Number(req.params.id), req.body);
...
const createTermAvailibility: RequestHandler = async (req, res, next) => {
  try {
    await caregiver.createTermAvailibility(req.body);
    res.status(200).json({ message: "Success" });
  } catch (err) {
    next(err);
  }
...
const deleteAvailibility: RequestHandler = async (req, res, next) => {
  try {
    await caregiver.deleteAvailibility(Number(req.params.availibility_id));
    res.status(200).json({ message: "Successfully deleted" });
  } catch (e) {
    next(e);
  }
